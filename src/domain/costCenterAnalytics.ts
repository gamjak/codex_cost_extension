...
function projectRows(entries: readonly FactDelta[], comparison: readonly FactDelta[], input: BuildCostCenterReportInput, total: number | undefined): CostCenterProjectRow[] {
...
export function buildCostCenterReport(input: BuildCostCenterReportInput): CostCenterReport {
  ...
  const summaryAggregate = aggregate(current); const comparisonAggregate = aggregate(comparison); const chart = buildChart(range, current, comparison); const sessions = sessionRows(current, comparison, range, summaryAggregate.cost); const projects = projectRows(current, comparison, input, summaryAggregate.cost); const models = modelRows(current, summaryAggregate.cost, input);
