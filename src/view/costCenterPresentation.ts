...
function styles(): string {
  return `:root { color: var(--vscode-editor-foreground); background: var(--vscode-editor-background); font-family: var(--vscode-font-family); } body { max-width: 1100px; margin: 0 auto; padding: 20px; line-height: 1.45; } button, select, input { font: inherit; } button { color: var(--vscode-button-foreground); background: var(--vscode-button-background); border: 1px solid var(--vscode-button-border, transparent); border-radius: 2px; padding: 6px 10px; cursor: pointer; } select, input { color: var(--vscode-input-foreground); background: var(--vscode-input-background); border: 1px solid var(--vscode-input-border); padding: 5px; } .lede, .notice { color: var(--vscode-descriptionForeground); } .controls, .filter-chips, [role="tablist"], .chart-points { display: flex; flex-wrap: wrap; gap: 8px; margin: 16px 0; align-items: center; } .metrics, .drivers { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 12px; } .drivers { grid-template-columns: repeat(3, minmax(0, 1fr)); } .metric, .driver, .budget { border: 1px solid var(--vscode-widget-border); background: var(--vscode-editorWidget-background); padding: 12px; border-radius: 4px; } dt { color: var(--vscode-descriptionForeground); } dd { font-size: 1.2rem; font-weight: 600; margin: 4px 0; } .chart-bar { fill: var(--vscode-charts-blue); } .chart-axis { stroke: var(--vscode-widget-border); } .chart-point { background: transparent; color: var(--vscode-textLink-foreground); padding: 2px; } :focus-visible { outline: 1px solid var(--vscode-focusBorder); outline-offset: 2px; } @media (max-width: 700px) { .metrics, .drivers { grid-template-columns: 1fr; } }`;
}
